...
// 🎯 FFMPEG-STATIC PATH (Heroku support)
...
export class AudioFeeder {
    ...
    constructor(sampleRate, channels, framesPerChunk, onChunk, source = "silence", onFinished, loop = true, warmupSilenceMs = 1500, trailingSilenceMs = 2000, loopGapMs = 1500) {
    ...
    static #decodeSource = (source, sampleRate, channels) => {
        ...
        return new Promise((resolve) => {
            ...
            // 🎯 FFMPEG PATH (fallback to system ffmpeg)
            const ffmpeg = ffmpegPath || "ffmpeg";
            console.log(`[AudioFeeder] Using ffmpeg: ${ffmpeg}`);
            ...
            if (!this.loop) {
                if (this.#trailingRemaining > 0) {
                    ...
                    if (!this.#finishedEmitted) {
                        ...
                        this.onChunk(frame);
                        console.log(`[AudioFeeder] Audio playback completed (${this.#chunksEmitted} chunks emitted).`);
                        this.stop();
                        this.onFinished?.();
                        return;
                    }
