...
export const makeMessagesSocket = (config) => {
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            // ── 🎯 DETECT BUTTONS (but don't convert yet) ─────────────────────
            const hasButtons = 
                typeof content === 'object' &&
                content !== null &&
            ...
            // ── 🎯 NOW: Convert buttons → interactiveMessage and INJECT ──────
            if (hasButtons) {
                ...
                // Build interactiveMessage using proto
                const interactiveMessage = proto.Message.InteractiveMessage.create({
                    ...
                    header: (content.header || content.title) ? proto.Message.InteractiveMessage.Header.create({
                        title: content.header || content.title,
                        hasMediaAttachment: false
                    }) : undefined
                });

                // Override the generated message with interactiveMessage
                fullMsg.message = { interactiveMessage };

                // Preserve contextInfo if present
                if (content.contextInfo) {
                    fullMsg.message.messageContextInfo = content.contextInfo;
                }

                logger.debug({ jid }, 'injected interactiveMessage with buttons');
            }
            // ─────────────────────────────────────────────────────────────────
