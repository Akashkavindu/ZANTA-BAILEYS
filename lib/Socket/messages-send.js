...
export const makeMessagesSocket = (config) => {
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            // ── 🎯 If has buttons, use custom flow ────────────────────────────
            if (hasButtons) {
                // Build native buttons
                const nativeButtons = content.buttons
                    .filter(btn => btn && (btn.buttonText || btn.text || btn.buttonId))
                    .slice(0, 3)
                    .map((btn, index) => {
                        const displayText = btn.buttonText?.displayText || btn.text || `Button ${index + 1}`;
                        const buttonId = btn.buttonId || btn.id || `btn_${index + 1}`;
                        const buttonType = btn.type || 1;

                        if (buttonType === 2 || buttonType === 'url') {
                            return {
                                name: 'cta_url',
                                buttonParamsJson: JSON.stringify({
                                    display_text: displayText.slice(0, 20),
                                    url: btn.url || btn.buttonText?.url || '',
                                    merchant_url: btn.url || btn.buttonText?.url || ''
                                })
                            };
                        } else if (buttonType === 3 || buttonType === 'call') {
                            return {
                                name: 'cta_call',
                                buttonParamsJson: JSON.stringify({
                                    display_text: displayText.slice(0, 20),
                        ...
                        } else if (buttonType === 4 || buttonType === 'copy') {
                            return {
                                name: 'cta_copy',
                                buttonParamsJson: JSON.stringify({
                                    display_text: displayText.slice(0, 20),
                                    copy_code: btn.copyCode || btn.code || ''
                                })
                            };
                        } else {
                            return {
                ...
                // Build interactiveMessage with proper image upload
                let headerConfig = undefined;
                let mediaMessage = null;

                if (content.image || content.video) {
                    try {
                        const mediaContent = content.image
                            ? { image: content.image }
                            : { video: content.video };

                        const uploadedMedia = await prepareWAMessageMedia(mediaContent, {
                            upload: waUploadToServer,
                            logger,
                            mediaCache: config.mediaCache,
                            options: config.options
                        ...
                        mediaMessage = uploadedMedia.imageMessage || uploadedMedia.videoMessage;

                        if (mediaMessage) {
                            headerConfig = proto.Message.InteractiveMessage.Header.create({
                                title: content.header || content.title || '',
                                hasMediaAttachment: true,
                                imageMessage: uploadedMedia.imageMessage || undefined,
                                videoMessage: uploadedMedia.videoMessage || undefined
                            });
                            logger.debug({ jid }, 'uploaded media for interactive message');
                        }
                    } catch (err) {
                        logger.error({ jid, err: err.message }, 'failed to upload media for buttons');
                    }
                } else if (content.header || content.title) {
                    headerConfig = proto.Message.InteractiveMessage.Header.create({
                        title: content.header || content.title,
                ...
                const interactiveMessage = proto.Message.InteractiveMessage.create({
                    body: proto.Message.InteractiveMessage.Body.create({
                        text: content.text || content.caption || content.body || 'Please select:'
                    }),
                    footer: proto.Message.InteractiveMessage.Footer.create({
                        text: content.footer || 'ᴘᴏᴡᴇʀᴇᴅ ʙʏ ᴢᴀɴᴛᴀ-ᴍɪɴɪ'
                    }),
                ...
                // 🎯 Build messageContextInfo with EVERYTHING
                const messageContextInfo = {};

                // ── 1. User provided contextInfo (forwarded, newsletter, mentions, etc.) ──
                if (content.contextInfo && typeof content.contextInfo === 'object') {
                    Object.assign(messageContextInfo, content.contextInfo);
                    logger.debug({ jid, ctxKeys: Object.keys(content.contextInfo) }, 'applied content.contextInfo');
                }

                // ── 2. Forwarded tag from content.forwarded or options.forwarded ──
                if (content.forwarded || options.forwarded) {
                    messageContextInfo.forwardingScore = content.contextInfo?.forwardingScore || 999;
                    messageContextInfo.isForwarded = content.contextInfo?.isForwarded ?? true;
                    logger.debug({ jid }, 'applied forwarded tag');
                }

                // ── 3. Quoted message (reply) — options.quoted ──
                const quotedMsg = options.quoted || content.quoted;
                if (quotedMsg && quotedMsg.key && quotedMsg.message) {
                    messageContextInfo.stanzaId = quotedMsg.key.id;
                    messageContextInfo.participant = quotedMsg.key.participant || quotedMsg.key.remoteJid;
                    messageContextInfo.quotedMessage = quotedMsg.message;
                    logger.debug({ jid, quotedId: quotedMsg.key.id }, 'applied quoted message');
                }

                // ── 4. Mentions ──
                if (content.mentions && Array.isArray(content.mentions) && content.mentions.length > 0) {
                    messageContextInfo.mentionedJid = content.mentions;
                    logger.debug({ jid, mentionCount: content.mentions.length }, 'applied mentions');
                }

                // ── 5. Additional options.contextInfo ──
                if (options.contextInfo && typeof options.contextInfo === 'object') {
                    Object.assign(messageContextInfo, options.contextInfo);
                    logger.debug({ jid }, 'applied options.contextInfo');
                }

                // Create fullMsg manually
                const msgId = generateMessageIDV2(sock.user?.id || userJid);
                const fullMsg = {
                    key: {
                        remoteJid: jid,
                        fromMe: true,
                ...
                // 🎯 Attach messageContextInfo if any keys exist
                if (Object.keys(messageContextInfo).length > 0) {
                    fullMsg.message.messageContextInfo = messageContextInfo;
                    logger.debug({
                        jid,
                        keys: Object.keys(messageContextInfo)
                    }, 'attached messageContextInfo to interactive message');
                }

                // Send via relayMessage
                ...
                logger.debug({
                    ...
                    hasQuoted: !!quotedMsg,
                    hasForwarded: !!messageContextInfo.forwardingScore
                }, 'sent interactiveMessage with buttons');
