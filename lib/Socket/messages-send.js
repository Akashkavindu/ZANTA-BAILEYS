    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (hasButtons) {
                ...
                // If image/video is provided, upload it first
                if (content.image || content.video) {
                    try {
                        ...
                        // Upload media using prepareWAMessageMedia
                        ...
                        // Get the uploaded message (imageMessage or videoMessage)
                ...
                // ── 🎯 Handle quoted/reply AND forwarded tags ────────────────
                const additionalNodes = [];

                // Case 1: User provided quoted message (reply)
                if (options.quoted || content.quoted) {
                    const quotedMsg = options.quoted || content.quoted;
                    
                    // Build contextInfo with quoted message info
                    const quotedContextInfo = {
                        stanzaId: quotedMsg.key?.id,
                        participant: quotedMsg.key?.participant || quotedMsg.key?.remoteJid,
                        quotedMessage: quotedMsg.message
                    };
                    
                    // Add to interactiveMessage's contextInfo
                    fullMsg.message.messageContextInfo = {
                        ...(fullMsg.message.messageContextInfo || {}),
                        ...quotedContextInfo
                    };
                    
                    logger.debug({ jid, quotedId: quotedMsg.key?.id }, 'adding quoted message context');
                }

                // Case 2: Forwarded message (forwardingScore)
                if (content.contextInfo?.forwardingScore || content.forwarded || options.forwarded) {
                    const forwardingScore = content.contextInfo?.forwardingScore || 999;
                    const isForwarded = content.contextInfo?.isForwarded ?? true;
                    ...
                    logger.debug({ jid, forwardingScore }, 'adding forwarding tag');
                }

                // Case 3: User provided full contextInfo
                if (content.contextInfo || options.contextInfo) {
                    const ctx = content.contextInfo || options.contextInfo;
                    fullMsg.message.messageContextInfo = {
                        ...(fullMsg.message.messageContextInfo || {}),
                        ...ctx
                    };
                ...
                logger.debug({ jid, hasMedia: !!mediaMessage, buttonCount: nativeButtons.length }, 'sent interactiveMessage with buttons');
                return fullMsg;
            }
            // ─────────────────────────────────────────────────────────────────
