...
export const makeMessagesSocket = (config) => {
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            // ── 🎯 DETECT BUTTONS (but REMOVE from content before generateWAMessage) ──
            const hasButtons = 
                typeof content === 'object' &&
                content !== null &&
            ...
            // 🎯 CRITICAL: Remove buttons from content before passing to generateWAMessage
            // Otherwise Baileys will wrap it in viewOnceMessageV2 and lose everything
            let contentForGenerate = content;
            if (hasButtons) {
                contentForGenerate = { ...content };
                delete contentForGenerate.buttons;
                logger.debug({ jid }, 'removed buttons from content before generateWAMessage');
            }
            // ─────────────────────────────────────────────────────────────────

            // ── Regular message ───────────────────────────────────────────────
            const fullMsg = await generateWAMessage(jid, contentForGenerate, {
                logger,
                userJid,
                getUrlInfo: (text) =>
                    getUrlInfo(text, {
            ...
            // ── 🎯 CONVERT buttons → interactiveMessage and INJECT ────────────
            if (hasButtons) {
                ...
                // 🎯 Extract image/video from generateWAMessage's output
                ...
                // 🎯 Extract contextInfo from generateWAMessage's output
                // (contains quoted, forwarded, newsletter)
                ...
                // 🎯 Replace fullMsg.message with interactiveMessage
                // BUT preserve contextInfo (quoted, forwarded, newsletter)
                const newMessage = {
                    interactiveMessage: interactiveMessage
                };

                // ✅ CRITICAL: Preserve messageContextInfo
                if (existingContextInfo && Object.keys(existingContextInfo).length > 0) {
                    newMessage.messageContextInfo = existingContextInfo;
                    logger.debug({
                        jid,
                        ctxKeys: Object.keys(existingContextInfo)
                    }, 'preserved messageContextInfo');
                }
                ...
                logger.debug({
                    ...
                    hasContextInfo: !!existingContextInfo
                }, 'injected interactiveMessage with buttons');
            }
            // ─────────────────────────────────────────────────────────────────
