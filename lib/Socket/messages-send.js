...
export const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            // ── Biz node for button messages (from Code 2) ────────────────────
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            // ── 🎯 NEW: buttons → interactiveMessage conversion (from Code 2) ────
            if (
                typeof content === 'object' &&
                content !== null &&
                Array.isArray(content.buttons) &&
                content.buttons.length > 0 &&
                !content.interactiveMessage // Don't convert if already interactive
            ) {
                const nativeButtons = content.buttons
                    ...
                    .map((btn, index) => {
                        ...
                        if (buttonType === 1 || buttonType === 'quick_reply') {
                            return {
                                name: 'quick_reply',
                        ...
                        } else if (buttonType === 2 || buttonType === 'url') {
                            return {
                                name: 'cta_url',
                                buttonParamsJson: JSON.stringify({
                                    display_text: displayText.slice(0, 20),
                ...
                if (content.header || content.title) {
                    interactiveContent.interactiveMessage.header = {
                ...
                if (content.contextInfo) mergedOptions.contextInfo = content.contextInfo;
                if (content.mentions) mergedOptions.mentions = content.mentions;
                if (content.quoted) mergedOptions.quoted = content.quoted;

                content = interactiveContent;
                options = mergedOptions;
                logger.debug({ jid }, 'converted standard buttons to interactiveMessage');
            }
            // ── END BUTTON CONVERSION ─────────────────────────────────────────
