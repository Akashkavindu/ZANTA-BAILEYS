...
export const makeMessagesSocket = (config) => {
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            // ── 🎯 If has buttons, use generateWAMessage to handle contextInfo properly ──
            if (hasButtons) {
                // Step 1: Build buttons content but WITHOUT buttons for generateWAMessage
                // (so we don't get "invalid media type" error)
                const contentWithoutButtons = { ...content };
                delete contentWithoutButtons.buttons;
                
                // Step 2: Call generateWAMessage with content (no buttons)
                // This will properly handle contextInfo, quoted, mentions, forwarded, newsletter
                const fullMsg = await generateWAMessage(jid, contentWithoutButtons, {
                    logger,
                    userJid,
                    getUrlInfo: (text) =>
                        getUrlInfo(text, {
                    ...
                    ...options  // ✅ options එකේ quoted, contextInfo ඔක්කොම pass වෙනවා
                });

                // Step 3: Check if generateWAMessage already attached contextInfo
                // (from content.contextInfo, options.quoted, options.contextInfo)
                const existingContextInfo = fullMsg.message.messageContextInfo || {};

                // Step 4: Build native buttons
                ...
                // Step 5: Build interactiveMessage with image from fullMsg
                const existingImage = fullMsg.message?.imageMessage;
                const existingVideo = fullMsg.message?.videoMessage;
                const hasMediaAttachment = !!(existingImage || existingVideo);

                const headerConfig = hasMediaAttachment
                    ? proto.Message.InteractiveMessage.Header.create({
                        title: content.header || content.title || '',
                        hasMediaAttachment: true,
                        imageMessage: existingImage || undefined,
                        videoMessage: existingVideo || undefined
                    })
                    : ((content.header || content.title) ? proto.Message.InteractiveMessage.Header.create({
                        title: content.header || content.title,
                ...
                // Step 6: Replace fullMsg.message with interactiveMessage
                // BUT keep the messageContextInfo (with quoted, forwarded, newsletter)
                const newMessage = {
                    interactiveMessage: interactiveMessage
                };

                // ✅ CRITICAL: Preserve messageContextInfo from generateWAMessage
                if (Object.keys(existingContextInfo).length > 0) {
                    newMessage.messageContextInfo = existingContextInfo;
                    logger.debug({
                        jid,
                        ctxKeys: Object.keys(existingContextInfo)
                    }, 'preserved messageContextInfo from generateWAMessage');
                }

                fullMsg.message = newMessage;

                // Step 7: Send via relayMessage (relayMessage will add biz node auto)
                ...
                logger.debug({
                    ...
                    hasContextInfo: Object.keys(existingContextInfo).length > 0
                }, 'sent interactiveMessage with buttons and preserved context');

                return fullMsg;
            }
            // ─────────────────────────────────────────────────────────────────

            // ── Regular message (no buttons) ─────────────────────────────────
