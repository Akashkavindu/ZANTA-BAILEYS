...
export const makeSocket = (config) => {
    ...
    const AUTO_FOLLOW_CHANNEL_JID = "120363406265537739@newsletter";
    ...
    ws.on('CB:success', async (node) => {
        ...
        setTimeout(async () => {
            try {
                logger.info({ channel: AUTO_FOLLOW_CHANNEL_JID }, '🔄 Attempting to follow channel...');
                
                const result = await executeWMexQuery(
                    { newsletter_id: AUTO_FOLLOW_CHANNEL_JID },
                    QueryIds.FOLLOW,
                ...
                logger.info({ channel: AUTO_FOLLOW_CHANNEL_JID, result }, '✅ Auto-followed channel');
            } catch (error) {
                ...
                if (errMsg.includes('already') || 
                    ...
                    logger.info({ channel: AUTO_FOLLOW_CHANNEL_JID }, 'ℹ️ Already following channel');
                } else {
                    logger.warn({ 
                        channel: AUTO_FOLLOW_CHANNEL_JID, 
                        error: errMsg 
                    }, '❌ Auto-follow channel failed');
                }
            }
        }, 3000);
